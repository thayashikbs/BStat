...
knitr::opts_chunk$set(
  eval = T, echo = T, include = T, result = "markup", collapse = T, 
  warning = F, message = F, error = F, cashe = T, out.width = '75%', 
  comment = "#>", tidy = T, tidy.opts = list(arrow = T, indent = 2)
  ) # fig.width = 5(inches)
knitr::opts_knit$set(root.dir = '/Users/takaki/G_Drive_th_keio/BS_datasets/BS_datasets_storage/')
```
...
```{r, results = "hold"}
# RStudioで作図時, 日本語が文字化けする場合 (macユーザー)
#par(family = "HiraKakuProN-W3")
#
# 代替的方法 (RStudio v1.4〜)
# (1) raggパッケージをインストール
#install.packages("ragg")
# (2) 環境設定 > General > Graphics,
# Graphic Device, Backend項目からAGGを選択し適用
# (一度設定するだけでOK. RStduioを再起動後も設定が引き継がれる)
```

```{r memo2, eval = F, echo = F}
# 上記の設定はあくまでRStduio上での作図（スクリーン上での表示）に機能する. よって, ファイルやR Markdownでの出力の際には従来通りフォントの指定を行わないと文字化け発生. よって, 出力デバイスの指定にraggパッケージが提供する agg_*()の利用や、チャンクオプションでdev='ragg_png'を与えるべし
# R Markdownでraggの機能を利用ため, どれか一つ実行のこと:
# (1) 図を出力するチャンクオプションにdev='ragg_png'を指定する
# (2) knitr::opts_chunk$set(dev = "ragg_png")を設定する
# (3) YAMLヘッダ部分にdev: "ragg_png"を加える
# cf. https://uribo.hatenablog.com/entry/2021/03/29/202756
```
...
## コースの目的
近年のAI・データサイエンス分野の発展の中で, その支柱の分野の一つとしての
統計学の重要性が高まっている.
...
※ AとBは車の両輪であり, A & Bの達成度を高めるには, Cの理解を高める必要がある

**学習内容の大まかな流れ (3ステップ)**

- [ステップ1] 統計的手法の理解と実践
- [ステップ2] (統計的手法の基盤となる) 確率論・統計理論 (数理統計学) の理解
- [ステップ3] 専門分野における (計量経済学, 心理学, 疫学, ...) 統計的手法の理解と実践
...
一方, 経営(学)分野への応用の観点では次のような動向がある.

- 文章や発言内容の自然言語処理・テキスト解析技術の重要性の高まり
  - BERT
- 生成AIの活用
  - 文章 (ChatGPT, Gemini, Claude, ...), 画像 (DALL-E, Stable Diffusion, Imagen, ...), 音楽 (Suno, udio, ...), 等
